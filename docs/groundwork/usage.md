...
### Generated-core diagrams — `groundwork gen-diagram [--check <file>] <manifest.json> <logical>=<graph.json> ...`

`gen-diagram` produces the *picture* a design doc would otherwise draw by hand —
and gets the arrows right by construction. **Every solid edge is an induced-
subgraph edge READ from a graph, and every tier annotation is READ from a
graph** — neither can be typed from memory. The judgment a human adds (proposed
FIX edges, conceptual nodes) is declared in the manifest and always rendered
**dashed**, so a reader tells the computed core from the proposal at a glance,
and CI can gate the core half against drift. Output is byte-deterministic for a
given (manifest, graphs) input.

Where `assert` verifies a graph against a doc's *textual* claims, `gen-diagram`
generates the doc's boxes-and-arrows from the graph: the manifest chooses **which**
functions to show and how to lay them out; the graph decides **which arrows
between them are real**.

**The manifest.** A manifest is a single JSON object, decoded strictly
(`DisallowUnknownFields`, trailing data rejected — the same fail-closed
single-value guard `graph.json` gets):

| field | shape | meaning |
|---|---|---|
| `id` | string | names the diagram (e.g. `"CG-6"`); rides the provenance comment and is the token `--check` selects a block by. NOT a mermaid identifier — it may carry `-`. |
| `direction` | `LR`\|`RL`\|`TB`\|`BT`\|`TD` | flowchart direction. Empty/missing is an **error** (fail closed — no silent default). |
| `tiers` | bool | when true, append ` (tN)` to every resolved node's label, `N` read from the graph node's tier. |
| `nodes[]` | `{id, graph, fn, label}` | one function to draw. `id` is a mermaid identifier; `graph` binds it to a logical graph named on the CLI; `fn` resolves it (below); `label` is the box text (**required, non-empty**). |
| `subgraphs[]` | `{id, title?, nodes[]}` | a mermaid subgraph box grouping some manifest node ids. A node belongs to **at most one** subgraph. |
| `overlays.nodes[]` | `{id, label?, shape?}` | conceptual nodes, **never resolved** against any graph. `shape`: `rect` → `["…"]` (default), `db` → `[("…")]`, `circle` → `(("…"))`. An empty label is legal **only** with `circle` (renders `(( ))`); rect/db empty is an error. |
| `overlays.edges[]` | `{from, to, label?}` | **always-dashed** proposal edges (`-.->`, optional `\|"label"\|`). Endpoints are manifest node ids or overlay node ids. |

`nodes[].graph` is the multi-graph axis: one manifest can span several services'
maps (`svcA=a.json svcB=b.json`), and a core edge is only ever drawn **within**
one graph.

**Resolving `fn` — the shared `fqnres` rule.** A node's `fn` resolves against its
bound graph using [the same `internal/fqnres` resolver](#claims-files--groundwork-assert-graphjson-claimsjson)
`assert` uses, over the same universe (node FQNs ∪ every edge endpoint, so a
`boundary:*` pseudo-node is drawable as a core node). A diagram node names **exactly
one** function, so a plain `fn` must resolve to exactly one FQN: **zero → UNRESOLVED,
two or more → AMBIGUOUS**, both errors (fail closed). The `/regex/` form is *rejected*
for a diagram node — multi-match is meaningful for a claim endpoint or a focus set,
but a single node is one function. A resolved endpoint that is not a graph **node** (a
`boundary:*` pseudo-node) genuinely has no tier, so under `tiers: true` it simply gets
**no** ` (tN)` suffix — an honest omission, never a fabricated tier.

**Core-edge induction.** For every ordered pair of manifest nodes `(a, b)` —
self-pairs included, so a genuine recursive self-call draws a self-loop — bound to
the **same** graph, `a → b` is drawn iff `(fqn(a), fqn(b))` is in that graph's set
of **unique `(from, to)` pairs** (the same mode-blind basis `assert` counts on).
Nothing else produces a solid edge; cross-graph pairs are never drawn.

**The overlay guard.** An overlay (dashed) edge is a proposal — an arrow the graph
does *not* have. If an overlay edge's two endpoints both resolve to manifest nodes
bound to the **same** graph and that `(fqn, fqn)` pair **exists** in the graph, the
"proposal" is really the computed truth in disguise: it is refused as a **VERDICT**
(exit 1 — the graph now contradicts the manifest's judgment; CI must surface it).
Overlay edges that touch an overlay node, or span two graphs, are **exempt** (they
cannot restate a single-graph core edge).

**Emission order** (byte-exact, all ordering by manifest order or a sort on the
intrinsic from/to node index — never map iteration or a clock):

```
flowchart <direction>
    %% generated by gen-diagram from <id>/diagram.json @ <stamps> — do not hand-edit the core
    <subgraph blocks, manifest order — intra-subgraph core edges, then undeclared members, all INSIDE the block>
    ...
    <overlay edges, manifest order — always dashed>
    <overlay nodes untouched by any overlay edge>
```

`<stamps>` is the sorted unique non-empty `tool` values of the input graphs, joined
by `, `, or the literal `unstamped` when none carry one (the committed goldens strip
`tool`). A node's bracket declaration is emitted **exactly once**, at first mention;
later mentions are the bare id. Every subgraph member is declared **inside** its
block (mermaid decides membership by declaration position). A label is emitted inside
double quotes with the one deterministic escape `"` → `&quot;`.

**`--check <file>` — the drift gate.** Instead of printing, `--check` regenerates
and byte-compares against a committed copy (regenerate-and-diff — the invariant
verifies itself). Selection is deterministic and fail-closed: if the file contains
any ` ```mermaid ` fenced blocks, the candidate is the one whose content carries the
provenance marker `generated by gen-diagram from <id>/` — **exactly one** must exist
(0 or ≥2 → operational error, exit 2), and the compare is against that block's inner
content; if the file has **no** mermaid fences, the whole file is compared
byte-for-byte (trailing newline included). A match prints nothing (exit 0); a drift
is a **verdict** (exit 1) naming the first differing line.

**Exit codes** ride groundwork's split, mirroring `assert`: **exit 1** for a
computed verdict (an overlay-guard violation, a `--check` drift); **exit 2** for an
operational error (bad flags, unreadable/undecodable inputs, an unresolved/ambiguous/
regex `fn`, a manifest validation failure, a `--check` block-selection failure);
**exit 0** on a clean generation or check.

**Worked example.** The committed manifest
`testdata/groundwork/diagrams/loansvc-core.diagram.json` draws the loan flow over
the loansvc golden graph — a subgraph (`origination`), an overlay `db` node + a
labeled dashed FIX edge (a *planned* audit ledger), and two `boundary:db …`
endpoints drawn as core nodes:

```
groundwork gen-diagram testdata/groundwork/diagrams/loansvc-core.diagram.json \
    loansvc=testdata/groundwork/goldens/loansvc.graph.json
```

emits (byte-pinned as `testdata/groundwork/goldens/loansvc-core.diagram.mmd`):

```mermaid
flowchart LR
    %% generated by gen-diagram from LOANSVC-CORE/diagram.json @ unstamped — do not hand-edit the core
    subgraph origination["origination [computed]"]
        CR["App.Create"] --> EV["Evaluator.Evaluate"]
        EV --> CH["Gateway.Charge"]
        ST["App.Status"]
    end
    ST --> SL["Loans.SelectLoan"]
    SL --> DBSEL["db SELECT loans"]
    OS["Payments.OnSettled"] --> MP["Loans.MarkPaid"]
    MP --> DBUPD["db UPDATE loans"]
    MP -.->|"FIX: append to audit ledger"| AUDIT[("audit_ledger (planned)")]
```

Every solid edge above (`App.Create → Evaluator.Evaluate`, `Loans.SelectLoan →
db SELECT loans`, …) is a real edge in the loansvc graph; the only hand-authored
arrow is the dashed `FIX` to the planned audit ledger. Commit the `.mmd` (or a doc
embedding it in a fence) and gate it in CI with
`groundwork gen-diagram --check testdata/groundwork/goldens/loansvc-core.diagram.mmd
<manifest> loansvc=<graph>`: the moment the graph loses `App.Create → Evaluate`, the
committed picture stops matching and the check fails — the diagram cannot silently
