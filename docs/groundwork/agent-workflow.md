...
There are two distinct three-valued systems — don't conflate them. Each has a
positive pole, a negative pole with a named witness, and a third "the graph has
nothing to say" pole, and in both the third pole is the whole point:

- **Obligation verdicts** (path obligations, partial-effect facts):
  **SATISFIED** — the property holds over every path, a universal proof
  stronger than any test; **VIOLATED** — broken, with a named witness (the
  leaking exit, the forbidden edge); **CANT-PROVE** — reachability could not
  answer.
- **Review verdicts** (`groundwork review` / `verify`):
  **STRUCTURALLY-CLEAR** — no new structural problem; **BLOCK** — a new
  violation, scope escape, or breaking contract, with the witness; and
  **NO-STRUCTURAL-SIGNAL** — the graph has nothing to say about this change.

**The third pole is not a pass.** Do not treat CANT-PROVE or
NO-STRUCTURAL-SIGNAL as "looks fine." It means reachability could not answer —
often because a blind spot (dynamic dispatch, a framework seam) sits on the
path. Surface it; don't silently proceed as if it were green.
