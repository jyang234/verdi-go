...
| [`graph-claims-drift-plan.md`](design/graph-claims-drift-plan.md) | `ACTIVE` | Plan for the 2026-07-03 consumer feature requests: `groundwork assert` (claims files verified against a graph, fail-loud resolution), attribute-aware `--diff` (third *changed* class + JSON delta), edge-record-multiplicity docs, and `--mermaid --focus` induced subgraphs sharing one resolver. |
