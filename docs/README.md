...
| [`schema-drift-check-plan.md`](design/schema-drift-check-plan.md) | `PROPOSAL` | Build plan for §1 of the headroom analysis: a `flowmap schema-drift` post-process diffing IR db-write labels against migration-defined schema. Package shape (modeled on `frontier`), soundness/completeness contract, fixture + test obligations, phasing. |
