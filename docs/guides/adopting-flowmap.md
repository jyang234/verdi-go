...
**Labeling outbound calls through a spec-generated client.** A client generated by
oapi-codegen (or a comparable generator) assembles its route dynamically
(`fmt.Sprintf` over path params), so the constant-fold HTTP labeler cannot name the
call — it falls through as an unnamed edge or an `UnresolvedSpecOperation` blind
spot. Declare the client package, the peer it addresses, and its OpenAPI document
under `classify.openapiClients`, then opt in at build time with `--reclaim-openapi`:

```yaml
classify:
  openapiClients:
    - package: example.com/wrapclientlib/eventbus   # the generated-client import path
      peer: event-bus                               # the boundary-label peer token
      spec: api/event-bus.openapi.yaml               # OpenAPI doc, relative to this file
      followWrappers: true    # opt-in: descend hand-written wrappers to the op (default false)
```

A call whose callee is a generated operation function in the declared package is
named `boundary:<peer> <METHOD> <template>`, tagged `via=openapi-client` so a
spec-asserted label is never mistaken for a discovered constant. A declared-package
callee that matches no operation is disclosed as an `UnresolvedSpecOperation` blind
spot naming the callee FQN, never guessed. `package`, `peer`, and `spec` are all
required (a blank one is a silently-disabled hint, refused at load); a
missing/unparseable/empty spec fails the build at the named config entry, before any
output.

**Descending hand-written wrappers (`followWrappers`, default `false`).** A real
fleet rarely calls the generated methods directly — the service calls a
hand-written wrapper that calls them, and without this every such call falls
through as `UnresolvedSpecOperation`. Setting it `true` on one hint is gated on
**config alone**, so effect 1 below runs on *every* flowmap command for the
service (not only a build that also passes `--reclaim-openapi`), and graph output
still stays a pure function of `(source, config)`:

1. **Widens the SSA build horizon.** The declared package's function bodies are
   materialized even when it lives in a separate module — otherwise it is a
...
**RTA/VTA interaction.** Widening the horizon (effect 1 above) can inflate
*unrelated* interface-dispatch blind spots under the default `rta`:
newly-materialized closures inside the client package become signature-matching
candidates for RTA's over-approximated interface resolution — the same
coarseness the closure-fan-out note in step 2 above describes. Measured in the
field: 10 → 377 blind spots on one interface-dense service; `--algo vta` resolved
it (10 → 9, exactly the expected −1 from the newly-named edge). That is a
precision trade-off, not a defect — pair `followWrappers` with `--algo vta` on
interface-dense services.
