...
- The `(site, kind)` must match a detected blind spot. `kind` may be omitted when the site has exactly one; a multi-kind site requires it; an annotation matching no blind spot **fails the build** (a stale FQN cannot silently attach to a vanished site). The binding rule is `config.ResolveAnnotationKind`, shared with the read-only MCP `annotate` proposer so a proposal it accepts is one the build accepts.
  - **One tolerated mismatch — an algorithm-fragile kind (§22).** The blind-spot manifest is `--algo`-dependent: a func-value `UnresolvedCall`/`ConcurrentDispatch` can fire under `vta` but be absent under the CLI-default `rta` (their presence is read off the algorithm's call-graph resolution; `blindspots.AlgoFragile`). An annotation naming such a kind, absent under *this* `--algo` at a site that is otherwise **live** (still carries another blind spot — so it is an `--algo` skew, not a stale FQN), is **warn-and-skipped**, not failed: it is dropped from the bound `annotations`, recorded on `graphio.Graph.SkippedAnnotations` (in-memory, `json:"-"` — no golden churn), and the CLI prints a stderr warning naming the site, kind, and algo. A hard build failure on a disclosure-only note that a different `--algo` would surface is worse than the lost note. This is the **only** relaxation: a mismatch on an algo-**stable** kind (`ExternalBoundaryCall` — a known external leaf, verified present under both `rta` and `vta`), or an orphan site, still fails closed. The producer signals the live-but-absent case as a typed `config.KindAbsentError`; the MCP `annotate` proposer mirrors the build exactly — it binds what the build binds, hard-rejects what the build fails on, and returns *algorithm-dependent guidance* (not a snippet) for the fragile-absent case the build warn-and-skips, so it never rejects what the build tolerates nor vice versa.
    - **Limit of the relaxation.** The skip requires the site to stay **live** (carry *another* blind spot) under the dropping algo. A site whose **only** blind spot was the fragile kind has zero blind spots once the algo drops it — indistinguishable from a stale FQN — so it takes the orphan path and **fails the build**, even though the cause is an `--algo` skew. This is deliberate (fail closed): skipping a fragile-kind annotation at a zero-blind-spot site would also swallow a genuine typo. Pin `--algo` (build under the algo that surfaces the kind); do not expect the skip to cover a fragile-only site.
