...
- **Distinct from any future E2E capture.** The same logical flow captured in-process (mocked deps) and later out-of-process (real deps, real downstream spans, real latency) produces *different* snapshots. They are distinct artifacts with distinct scopes — don't make one golden serve both, or you recreate the identity drift the consistency pass cleaned up.
