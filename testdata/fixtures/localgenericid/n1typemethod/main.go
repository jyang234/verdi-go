...
// TYPE. Show declares no type parameters of its own and there is no generic
// FUNCTION anywhere in the program, yet Box's parameter makes the analyzer build
// Show's body once per instantiation of Box — so conjunct 1 of the blast radius
// is about a body the analyzer builds more than once, not about the `func f[X]`
// syntax.
//
