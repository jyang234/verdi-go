...
// crossServiceFixture is a committed 2-resource OTLP/JSON trace: impeachsvc (entry +
// outbound client) propagating into peersvc (server + two DB DELETEs). The peer's clock
// is skewed ~0.5s behind impeachsvc's, and its two sibling DB writes are timestamped
